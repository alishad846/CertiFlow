...
function ImageCollectionTab({ onClose }: { onClose: () => void }) {
  ...
  const loadData = useCallback(
    async (offset = 0, kw = '') => {
      // Stock images are only shown once the user searches — an empty query loads nothing.
      if (!kw.trim()) {
        setIsLoading(false);
        return;
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isLoading && !keyword.trim() && images.length === 0 && (
          <div
            css={{
              gridColumn: '1 / -1',
              textAlign: 'center',
          ...
          >
            Search for stock images to add to your design.
          </div>
        )}
        {!isLoading && keyword.trim() && images.length === 0 && (
          <div
          ...
          >
            No stock images found for “{keyword}”.
          </div>
        )}
      </div>
    </div>
