    ...
    return useMemo(
        () => ({
            getResized: (
                ...
                // applies the full diagonal distance to both axes then re-locks the ratio, which
                // overshoots badly on non-square boxes (the corner flies away from the cursor).
                const CORNERS: Direction[] = ['topLeft', 'topRight', 'bottomLeft', 'bottomRight'];
                if (lockAspect && CORNERS.includes(direction) && Math.abs(rotate % 360) < 0.01) {
                    const w = boxSize.width;
                    const h = boxSize.height;
                    const dx = (newPosition.clientX - original.clientX) / (frameScale || 1);
                    const dy = (newPosition.clientY - original.clientY) / (frameScale || 1);
                    const sx = direction === 'topLeft' || direction === 'bottomLeft' ? -1 : 1;
                    const sy = direction === 'topLeft' || direction === 'topRight' ? -1 : 1;
                    const outX = dx * sx;
                    const outY = dy * sy;
                    const s = 1 + (w * outX + h * outY) / (w * w + h * h);
                    let newWidth = Math.max(MIN_WIDTH, w * s);
                    let newHeight = Math.max(MIN_HEIGHT, h * s);
                    // Keep the aspect ratio exact even after the min clamps.
                    ...
                    const x = direction === 'topLeft' || direction === 'bottomLeft' ? right - newWidth : position.x;
                    const y = direction === 'topLeft' || direction === 'topRight' ? bottom - newHeight : position.y;
                    return {
