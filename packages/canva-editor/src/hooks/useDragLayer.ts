...
const getChange = (
  ...
  if (controlBox && controlBox) {
    ...
    // Snap against every other top-level layer (edge↔edge, center↔center, edge↔center).
    ...
    // Also snap to the PAGE itself — its edges and, most importantly, its center — so a single
    // element can be centered on the canvas even when there is nothing else to align to.
    const root = layers['ROOT'];
    if (root) {
      const pw = root.data.props.boxSize.width;
      const ph = root.data.props.boxSize.height;
      checkTarget({
        top: 0,
        left: 0,
        right: pw,
