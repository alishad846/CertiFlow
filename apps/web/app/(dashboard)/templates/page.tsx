            ...
            <Card key={template.id} className="border-white/80 overflow-hidden">
              <div className="grid gap-4 lg:grid-cols-[220px,1fr]">
                <div className="overflow-hidden rounded-[22px] border border-slate-200 bg-slate-50 min-h-[220px]">
                  {template.backgroundUrl.toLowerCase().endsWith('.pdf') ? (
                    <object
                      data={`${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:4000'}${template.backgroundUrl}#toolbar=0&view=FitH`}
                      type="application/pdf"
                      className="h-full w-full"
                    >
                      <div className="flex h-full min-h-[220px] items-center justify-center bg-slate-100 text-sm font-semibold text-slate-500">
                        PDF template
                      </div>
                    </object>
                  ) : (
