...
export function SecuritySection({ user, onSaved }: Props) {
  ...
  useEffect(() => {
    if (typeof user?.twoFactorEnabled === 'boolean') setEnabled(user.twoFactorEnabled);
    else apiFetch<{ user: { twoFactorEnabled?: boolean } }>('/auth/me').then((d) => setEnabled(Boolean(d.user.twoFactorEnabled)));
  }, [user]);
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6">
        <h3 className="font-serif text-2xl text-ink">Two-factor authentication</h3>
          ...
          {!setup ? (
            ...
            <div className="grid gap-6 md:grid-cols-[auto_1fr] md:items-start">
              ...
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-ink-soft">Scan the QR with your authenticator app, or enter this key manually:</p>
                ...
                <form onSubmit={confirmEnable} className="mt-4 flex flex-col gap-3 sm:flex-row">
                  <Input value={code} onChange={(e) => setCode(e.target.value)} inputMode="numeric" placeholder="6 digit code" className="sm:max-w-[200px]" />
      ...
      {enabled === true ? (
        <Card className="p-6">
          ...
          <form onSubmit={disable} className="mt-4 flex flex-col gap-3 sm:flex-row">
            <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Your password" className="sm:max-w-[280px]" />
